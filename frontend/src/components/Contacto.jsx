...
const Contacto = () => {
  ...
  return (
    <section id="contacto" className="py-32 bg-slate-50 relative overflow-hidden">
      ...
      <div className="absolute bottom-0 left-0 w-80 h-80 bg-[#9ABE00]/5 rounded-full blur-[120px] -ml-40 -mb-40 z-0" />
        ...
        <div className="grid lg:grid-cols-12 gap-20 items-start">
          ...
          <div className="lg:col-span-5">
            ...
            >
              ...
              <h2 className="text-5xl md:text-7xl font-display font-black text-[#002b45] uppercase leading-[0.9] tracking-tighter mb-10">
                ...
                <span className="text-[#9ABE00] italic">{(contacto.titulo || '').split(' ').slice(1).join(' ')}</span>
              </h2>
              <p className="text-xl text-slate-500 mb-16 leading-relaxed border-l-4 border-[#9ABE00] pl-8 font-medium">
          ...
          <div className="lg:col-span-7">
            <motion.div
              ...
              viewport={{ once: true }}
              className="bg-[#002b45] p-10 lg:p-20 shadow-2xl relative overflow-hidden rounded-sm border-b-[10px] border-[#9ABE00]"
            >
              {/* Internal decorative elements */}
              <div className="absolute top-0 right-0 w-32 h-32 bg-white/5 rounded-full -mr-16 -mt-16 blur-2xl" />
              <div className="absolute bottom-0 left-0 w-48 h-48 bg-[#9ABE00]/5 rounded-full -ml-24 -mb-24 blur-3xl" />
              
              <div className="relative z-10">
                <div className="flex items-center gap-4 text-[#9ABE00] text-[11px] font-black tracking-[0.4em] uppercase mb-12 border-b border-white/10 pb-6">
                ...
                <form className="space-y-10" onSubmit={handleWhatsAppSend}>
                  ...
                  <div className="space-y-4">
                    <label className="flex items-center gap-3 text-[11px] font-black uppercase tracking-[0.2em] text-slate-400">
                      <MessageSquare size={16} className="text-[#9ABE00]" /> MENSAJE O REQUERIMIENTO
                    </label>
                    <textarea 
                      name="mensaje"
                      ...
                      onChange={handleInputChange}
                      required
                      className="w-full bg-white/[0.03] border-b-2 border-white/10 p-5 text-white font-sans font-medium focus:border-[#9ABE00] focus:outline-none transition-all duration-500 min-h-[160px] resize-none hover:bg-white/10"
                  ...
                  <motion.button 
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="w-full bg-[#9ABE00] text-[#002b45] font-display font-black text-[12px] tracking-[0.3em] py-6 transition-all duration-300 flex items-center justify-center gap-4 uppercase shadow-[0_15px_30px_-10px_rgba(152,197,96,0.3)] rounded-sm group"
                  >
...
const ContactItem = ({ icon, title, content, link }) => (
  <a href={link} className="flex gap-8 items-center group">
    ...
    <div>
      ...
      <p className="text-xl font-display font-black text-[#002b45] group-hover:text-[#9ABE00] transition-colors tracking-tight italic">{content}</p>
    </div>
  </a>
);
...
const InputGroup = ({ label, placeholder, icon, name, value, onChange, type = "text", required = false }) => (
  <div className="space-y-4">
    <label className="flex items-center gap-3 text-[11px] font-black uppercase tracking-[0.2em] text-slate-400">
      <span className="text-[#9ABE00]">{icon}</span> {label}
    </label>
    <input 
      type={type}
      name={name}
      ...
      onChange={onChange}
      required={required}
      className="w-full bg-white/[0.03] border-b-2 border-white/10 p-5 text-white font-sans font-medium focus:border-[#9ABE00] focus:outline-none transition-all duration-500 hover:bg-white/10"
