      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col lg:flex-row items-end gap-10 mb-24">
          ...
          >
            ...
            <h2 className="text-5xl md:text-7xl font-display font-black text-[#002b45] leading-tight tracking-tight uppercase">
              {(header.titulo || '').split(' ')[0]} <span className="text-[#9ABE00] italic">{(header.titulo || '').split(' ').slice(1).join(' ')}</span>
            </h2>
            <motion.p 
        ...
        <div className="grid lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-8 space-y-12">
            <div className="grid sm:grid-cols-2 gap-8">
              {investigaciones.map((inv, idx) => (
                ...
                >
                  ...
                  <div className="w-14 h-14 bg-[#002b45] flex items-center justify-center text-white mb-8 group-hover:bg-[#9ABE00] group-hover:text-[#002b45] transition-all duration-300 relative z-10 shadow-lg">
                    <Microscope size={28} />
                  </div>
                  <h4 className="text-xl font-display font-black text-[#002b45] mb-4 uppercase tracking-tight relative z-10 leading-tight group-hover:text-[#9ABE00] transition-colors">{inv.titulo}</h4>
                  <p className="text-slate-500 text-sm leading-relaxed mb-8 relative z-10 line-clamp-3 font-medium">{inv.descripcion}</p>
                  <Link href={inv.link || "#"} className="flex items-center gap-3 text-[11px] font-display font-black tracking-[0.25em] text-[#002b45] uppercase group-hover:text-[#9ABE00] transition-all relative z-10">
            ...
            {/* Papers List */}
            {papers.length > 0 && (
              <div className="bg-[#002b45] p-12 text-white shadow-2xl relative overflow-hidden rounded-sm border-t-8 border-[#9ABE00]">
                ...
                <div className="flex justify-between items-center mb-10 border-b border-white/10 pb-8">
                  <h3 className="text-2xl font-display font-black flex items-center gap-4 uppercase tracking-widest">
                    <ScrollText className="text-[#9ABE00]" size={28} />
                    Publicaciones Científicas
                  </h3>
                  <Link href="/papers" className="text-[11px] font-black tracking-widest text-[#9ABE00] hover:text-white transition-colors uppercase border-b-2 border-[#9ABE00] pb-1">
                    Ver Todo
                  </Link>
                </div>
                <div className="space-y-10">
                  {papers.map((paper) => (
                    <div key={paper.id} className="group border-b border-white/5 pb-10 last:border-0 last:pb-0">
                      <span className="text-[#9ABE00] text-xs font-black tracking-[0.4em] uppercase block mb-4">{paper.anio}</span>
                      <h4 className="text-2xl font-display font-bold mb-4 group-hover:text-[#9ABE00] transition-colors leading-tight tracking-tight">{paper.titulo}</h4>
                      ...
                      <div className="flex items-center gap-5 text-[11px] font-black text-white/30 uppercase tracking-[0.2em]">
                        ...
                        <span className="w-1.5 h-1.5 bg-[#9ABE00] rounded-full" />
          ...
          <div className="lg:col-span-4 space-y-10">
            {patentes.length > 0 && (
              <div className="bg-[#002b45] p-12 shadow-2xl text-white relative overflow-hidden rounded-sm">
                ...
                <h3 className="text-2xl font-display font-black text-white mb-8 uppercase leading-tight relative z-10 tracking-tight">Patentes y <br /><span className="text-[#9ABE00]">Registros</span></h3>
                  ...
                  {patentes.map((patente) => (
                    <div key={patente.id} className="flex items-start gap-4 border-b border-white/5 pb-6 last:border-0">
                      <div className="w-8 h-8 bg-white/5 rounded-full flex items-center justify-center shrink-0">
                        <Lightbulb className="text-[#9ABE00]" size={18} />
                      </div>
            ...
            <div className="border-[6px] border-[#002b45] p-12 bg-white shadow-2xl relative overflow-hidden">
              ...
              <div className="space-y-5">
                <div className="flex items-center gap-4 group">
                  <div className="w-10 h-10 bg-slate-50 flex items-center justify-center text-[#9ABE00] group-hover:bg-[#9ABE00] group-hover:text-white transition-all duration-300 rounded-sm">
                    <Award size={22} />
                  </div>
